...
export const EmailVerificationScreen: React.FC = () => {
  ...
  return (
    <div id="email-verification-gate" className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 font-sans">
      <div className="bg-white border border-black max-w-md w-full p-8 relative flex flex-col gap-6 max-h-[92vh] overflow-y-auto shadow-2xl">
        ...
        <button
          ...
          className="absolute right-4 top-4 font-mono text-xs px-2 py-1 bg-zinc-100 hover:bg-zinc-200 border border-zinc-300 text-zinc-800 cursor-pointer"
        >
          [Выход]
        </button>
        ...
        {/* Target Email Box */}
        <div className="bg-zinc-50 border border-zinc-300 p-3.5 font-mono text-xs flex flex-col gap-1">
          <span className="text-[10px] uppercase tracking-wider text-zinc-400">
            Адрес получателя:
          </span>
          <span className="font-bold text-zinc-950 break-all text-sm">
            {user?.email || 'Ваш email'}
          </span>
        </div>

        {/* Spam instruction box matching AuthModal prompt style */}
        <div className="p-3 bg-zinc-100 border border-zinc-300 font-sans text-xs text-zinc-800 flex flex-col gap-1.5">
          <div className="font-mono font-bold text-[10px] uppercase tracking-wider text-zinc-950">
            [Инструкция по активации]
          </div>
          <p className="leading-relaxed">
            Мы отправили ссылку для активации. Откройте сообщение и нажмите на ссылку в письме.
          </p>
          <div className="p-2 bg-white border border-zinc-300 font-mono text-[11px] text-zinc-900 leading-snug">
        ...
        {infoMessage && (
          <div className="border border-black bg-zinc-100 p-3 font-mono text-xs text-zinc-950 flex flex-col gap-1">
        ...
        {errorMessage && (
          <div className="border border-zinc-400 bg-zinc-100 p-3 font-mono text-xs text-zinc-900 flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-3 font-mono text-xs">
          <button
            ...
            onClick={handleCheck}
            disabled={checking}
            className="w-full bg-black hover:bg-[#0033CC] text-white font-mono text-xs uppercase tracking-wider py-3.5 border border-black font-bold transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            disabled={resending}
            className="w-full bg-white hover:bg-zinc-100 border border-zinc-300 text-zinc-900 font-mono text-xs uppercase tracking-wider py-3 px-4 transition-colors cursor-pointer"
          >
        ...
        <div className="text-center font-mono text-xs text-zinc-500 pt-2 border-t border-zinc-200 flex items-center justify-center gap-1.5">
